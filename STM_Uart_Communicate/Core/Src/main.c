...
uint8_t rx_index = 0; // Index for buffer
uint8_t command_received = 0; // Flag to indicate command received
/* USER CODE END PV */
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
  if (huart->Instance == USART2) {
    if (rx_buffer[rx_index - 1] == '\n') { // Check for newline character
      rx_buffer[rx_index - 1] = '\0'; // Replace newline with null terminator
      command_received = 1; // Set flag
    } else {
      HAL_UART_Receive_IT(&huart2, (uint8_t*)&rx_buffer[rx_index++], 1); // Continue receiving
    }
  }
}
/* USER CODE END PFP */
